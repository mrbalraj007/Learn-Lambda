...
# Retrieves all groups a user belongs to and exports to a CSV file
# Author: AWS Engineer
...
# CSV header
echo "Username,Group ID,Group Name" > "$OUTPUT_CSV"
...
if [ "$GROUP_COUNT" -eq 0 ]; then
    echo "$username,N/A,No groups found" >> "$OUTPUT_CSV"
...
for group_id in $GROUP_IDS; do
    ...
    if [[ "$group_name" == *","* ]]; then
        echo "$username,$group_id,\"$group_name\"" >> "$OUTPUT_CSV"
    else
        echo "$username,$group_id,$group_name" >> "$OUTPUT_CSV"
    fi

    echo -e "${GREEN}✔ $username -> $group_id -> $group_name${NC}"
done

# Final display
echo -e "${BLUE}=== CSV File Contents ===${NC}"
cat "$OUTPUT_CSV"
echo -e "${GREEN}✅ Completed. Output saved to:${NC} ${YELLOW}$OUTPUT_CSV${NC}"
