...
CSV_REPORT_FILE="backup_jobs_report_${TIMESTAMP}.csv"
...
write_csv_header() {
  echo "Backup job ID,Status,Resource name,Message category,Resource ID,Resource type,Creation time" > "$CSV_REPORT_FILE"
}

write_csv_row() {
  local job_id="$1" status="$2" resource_name="$3" category="$4" resource_id="$5" resource_type="$6" creation_time="$7"
  echo "$(csv_escape "$job_id"),$(csv_escape "$status"),$(csv_escape "$resource_name"),$(csv_escape "$category"),$(csv_escape "$resource_id"),$(csv_escape "$resource_type"),$(csv_escape "$creation_time")" >> "$CSV_REPORT_FILE"
}
...
# New: Get EC2 instance "Name" tag (fallback to instance ID if missing)
get_instance_name() {
...
# New: Map AWS Backup Job Status to a message category
get_message_category() {
  local status="${1^^}"
  case "$status" in
    COMPLETED) echo "Success" ;;
    FAILED|ABORTED|EXPIRED) echo "Error" ;;
    PARTIAL|STOPPED) echo "Warning" ;;
    *) echo "Info" ;;
  esac
}
...
start_backup() {
  ...
  if ! validate_instance "$PROFILE" "$REGION" "$INSTANCE_ID"; then
    # Write a CSV row even if instance not found
    local now_utc
    now_utc=$(date -u +%FT%TZ)
    write_csv_row "" "FAILED_TO_START" "$(get_instance_name "$PROFILE" "$REGION" "$INSTANCE_ID")" "Error" "$INSTANCE_ID" "EC2" "$now_utc"
    return 1
  fi
  ...
  local RESOURCE_NAME
  RESOURCE_NAME="$(get_instance_name "$PROFILE" "$REGION" "$INSTANCE_ID")"
  ...
  if [[ $BACKUP_EXIT_CODE -eq 0 ]] && [[ -n "$BACKUP_JOB_ID" ]] && [[ "$BACKUP_JOB_ID" != "None" ]]; then
    ...
    # Describe the job to capture details for CSV
    JOB_STATUS=$(aws backup describe-backup-job --profile "$PROFILE" --region "$REGION" --backup-job-id "$BACKUP_JOB_ID" --query 'Status' --output text 2>/dev/null || echo "CREATED")
    JOB_RESOURCE_TYPE=$(aws backup describe-backup-job --profile "$PROFILE" --region "$REGION" --backup-job-id "$BACKUP_JOB_ID" --query 'ResourceType' --output text 2>/dev/null || echo "EC2")
    JOB_CREATION_TIME=$(aws backup describe-backup-job --profile "$PROFILE" --region "$REGION" --backup-job-id "$BACKUP_JOB_ID" --query 'CreationDate' --output text 2>/dev/null || date -u +%FT%TZ)
    MSG_CAT=$(get_message_category "$JOB_STATUS")
    write_csv_row "$BACKUP_JOB_ID" "$JOB_STATUS" "$RESOURCE_NAME" "$MSG_CAT" "$INSTANCE_ID" "$JOB_RESOURCE_TYPE" "$JOB_CREATION_TIME"
    rm -f "$ERROR_OUTPUT"
    return 0
  else
    ...
    # Write failure row to CSV
    local now_utc
    now_utc=$(date -u +%FT%TZ)
    write_csv_row "" "FAILED_TO_START" "$RESOURCE_NAME" "Error" "$INSTANCE_ID" "EC2" "$now_utc"
    rm -f "$ERROR_OUTPUT"
    return 1
  fi
}
