#!/usr/bin/env bash
# ============================================================================
# Script Name : backup_last24hrs.sh
# Description : Collect AWS Backup jobs from the last 24 hours across multiple
...
OUTPUT_FILE="backup-last24hrs-report_${TIMESTAMP}.csv"
SUMMARY_FILE="backup-last24hrs-summary_${TIMESTAMP}.csv"
...
# (remove CREATED_AFTER; not needed with by-created-after/by-complete-after)
...
while read -r PROFILE; do
  ...
  echo "Collecting backup jobs for profile: $PROFILE"
  ...
  # Gather all jobs created OR completed within last 24h, paginate both, then union by BackupJobId
  ALL_JOBS_JSON=$(
    {
      fetch_jobs_arrays "$PROFILE" "$REGION" --by-created-after "$START_TIME"
      fetch_jobs_arrays "$PROFILE" "$REGION" --by-complete-after "$START_TIME"
